...
def main(
    ...
    for feature_row, prediction_row in zip(feature_rows, prediction_rows):
        if feature_row.get("y") != prediction_row.get("y"):
            ...
            logger.warning("Target row mismatch detected between features and predictions")
    ...
    ordered_rows = sorted(series_rows, key=lambda pair: _time_sort_key(pair[0].get("timestamp", "")))
    x_values = list(range(len(ordered_rows)))
    y_true = [float(feature_row["y"]) for feature_row, _ in ordered_rows]
    q10 = [float(prediction_row[f"{model_name}_lo"]) for _, prediction_row in ordered_rows]
    q90 = [float(prediction_row[f"{model_name}_hi"]) for _, prediction_row in ordered_rows]
    y_pred = [float(prediction_row[f"{model_name}_pred"]) for _, prediction_row in ordered_rows]

    output_path.parent.mkdir(parents=True, exist_ok=True)

    plt.figure(figsize=(11, 5))
    plt.plot(x_values, y_true, label="Observed demand", color="#1f2937", linewidth=2)
    plt.plot(x_values, y_pred, label=f"{model_name} point forecast", color="#2563eb", linewidth=2)
    plt.fill_between(x_values, q10, q90, color="#60a5fa", alpha=0.25, label="10%-90% band")
