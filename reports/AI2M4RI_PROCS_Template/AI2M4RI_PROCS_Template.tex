...
%% \usepackage{amsthm}
\usepackage{graphicx}
% Compact float/caption/math spacing to respect strict page limits.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.84\linewidth]{../figures/Architecture.drawio.png}
...
To analyze scarcity sensitivity, Figure~\ref{fig:transfer-heatmap} reports the RMSE gain of transfer-regularized ridge over target-only ridge across history fractions and transfer regularization strengths. Positive values indicate improvement from transfer. As expected, the low-history regime remains the most informative for evaluating transfer benefits.

\begin{figure}[t]
\centering
\includegraphics[width=0.82\linewidth]{../figures/transfer_gain_heatmap.png}
\caption{RMSE gain of transfer-regularized ridge over target-only ridge across scarcity levels and $\lambda$ values.}
\label{fig:transfer-heatmap}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.86\linewidth]{../figures/test_series_confidence_band.png}
...
While the performance gain from transfer regularization is modest, its consistency across scarcity levels suggests that the approach provides a reliable safety net for early-stage replenishment decisions, precisely where more complex models tend to overfit.
